...
def init_(self):
self.csvpath='
self.template_dir=os.path.split(os.path.abspath(sys.argv[0])) [θ]+'/. ./../utils/' self.output_dir = os.path.split(os.path.abspath(sys.argv[0])) [θ]+'/../../utils/' def main(self):
if not self.csvpath:
...
self.template_dir += 'pkt_rdy_template'
